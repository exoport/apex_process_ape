...
// Exit codes specific to `ape framework update` failures so script
...
func newFrameworkCmd() *cobra.Command {
  ...
  ape framework update     Install/refresh skills and pipelines from a checked-out
                           framework repo, seeding _apex/config.yaml on first run.
  ape framework status     Inspect the installed framework version, with optional
                           drift report against the framework repo's HEAD.
...
func newFrameworkUpdateCmd(repoFlag, cwdFlag *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Install/refresh framework skills and pipelines into the project",
		Long: `Install (or refresh) framework-managed assets into <project>:
  ...
  - _apex/config.yaml      seeded if absent (interactive prompt by
                           default; supply --project-name and
                           --extensions to skip the TUI; --no-bootstrap
                           to skip seeding entirely)
  - _apex/framework.yaml   metadata recording what was installed.

Refuses to run if the framework repo is dirty, on a non-main branch,
or if the project has uncommitted changes under .claude/skills/apex-*.
...
'update' refuses to seed silently.`,
	...
	cmd.Flags().BoolVar(&noBootstrap, "no-bootstrap", false, "Skip _apex/config.yaml seeding entirely on first install")
	return cmd
}
