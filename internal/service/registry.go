...
// running so callers can't add a job in a terminal state by mistake.
...
// Finish records a job's terminal state from its child exit code. A job
// marked via RequestStop becomes "stopped"; otherwise exit 0 → "done" and
// any non-zero → "failed". No-op for an unknown id.
func (r *Registry) Finish(id string, exitCode int) {
	r.mu.Lock()
	defer r.mu.Unlock()
	e, ok := r.jobs[id]
	if !ok {
