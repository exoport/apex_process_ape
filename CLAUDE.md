...
- **Everything ape writes into a project goes under `_output/ape/`, and the paths live in `internal/runlog/layout.go` — nowhere else.** `_output/` belongs to the framework (its `output_folder` variable; the skills write handoffs, briefs and verify reports there). Never build a run path with `filepath.Join(projectRoot, "_output", …)`: call `runlog.PipelineRunDir`, `runlog.TasksRoot`, `runlog.RunRoots`, etc. Consumers that sweep every run **must** iterate `runlog.RunRoots` rather than listing roots themselves — a hand-written list is what made `hookdrift` miss `ape pipeline` runs entirely, and made two other consumers' coverage accidental. A new run kind is one line in `layout.go`.
