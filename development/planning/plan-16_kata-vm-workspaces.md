...
- [ ] **D4 public egress** — `ProxyEnv` wires `HTTPS_PROXY`/`HTTP_PROXY`/`NO_PROXY` into the guest and `sandbox up --proxy host:port` points it at a running CONNECT proxy (reused, unchanged). **Remaining:** a persistent host-side proxy *supervisor* started/stopped by `up`/`down` (a detached daemon) so a long-lived workspace has always-on egress — needs live Kata to validate, so deferred.
- [x] **D7 access** — sshd is in the image (key-auth-only, over the forwarded `--ssh-port` loopback); `sandbox ssh` builds the `ssh` invocation. *Follow-up: the composer doesn't yet drop `authorized_keys` into the staged home — use `attach`/`exec` until it does (noted in the image README).*
...
- [x] **Tests (Tier-1)** — composer, profile (incl. new fields), proxy, OCI-config builder, **nerdctl command construction + registry** (`kata_test.go`), doctor sandbox checks. Green under `-race`; Windows cross-compile + lint clean. Tier-2 kata integration is Step 6 (local/self-hosted only).
...
> 2. **Egress proxy lifecycle** is the one D4 gap (see above) — the wiring is done, the persistent supervisor daemon is not.
