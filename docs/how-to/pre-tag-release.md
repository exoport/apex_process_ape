...
| Emulator agreement (`emulators_agree`) | A real session's bytes rendered by x/vt and by the vt10x oracle must agree on every read ape makes — and, on the ready REPL, on the whole pane. A sequence one emulator misreads fails here with both panes and the bytes, not in a stalled run: claude 2.1.269's kitty query (vt10x) and claude's window title (x/vt) were both that. |
