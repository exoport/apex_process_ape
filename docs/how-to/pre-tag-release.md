...
| The empty `❯` prompt line (`❯` + U+00A0) | The fallback ready signal. It matched no real claude until this was asserted on a real pane — readiness rested on the footer alone. |
| Emulator agreement (`emulators_agree`) | A real session's bytes rendered by x/vt and by the vt10x oracle must agree on every read ape makes — and, on the ready REPL, on the whole pane. A sequence one emulator misreads fails here with both panes and the bytes, not in a stalled run: claude 2.1.269's kitty query (vt10x) and claude's window title (x/vt) were both that. Run twice, because claude writes two streams depending on the terminal its environment names and ape passes `TERM_PROGRAM` through: `kitty_push` (`TERM_PROGRAM=tmux` — kitty keyboard flags pushed around keypresses) and `query_only` (`TERM_PROGRAM=vscode` — query only, synchronized output). Each also asserts from its recorded bytes that claude really wrote that stream, so a claude that stops choosing this way fails the gate instead of testing one stream twice. |
| Background-shell reap switch (`bg_shell_reap_switch`) | ape sets `CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP=1` on every spawn so a background shell is not killed mid-step when Bun reports memory pressure. This reads the installed binary and fails if the variable is gone, if the `process.on("memoryPressure", …)` registration it guards is gone, or if the two are no longer within 4 KiB of each other — the three have different remedies. The reap only fires under real kernel pressure, so the coupling is what can be checked, not the behaviour. |
| The startup check (`startup_probe`) | The once-per-version check every user's first run performs must verify the installed claude. If it misjudges a working claude, every run on it is stopped at the start — this is where that is caught instead. |
| Repaints while a tool runs (`pty_repaints_during_tool`) | `ape prompt`'s idle anchor has exactly one live signal during a single silent tool call: PTY output. Measured in this gate — while `sleep 25` runs, claude repaints about once a second while the transcript does not grow by a byte and no hook fires between `PreToolUse` and `PostToolUse`. A claude that stopped repainting would turn that path's `--idle-timeout` into a cancel button for healthy long tool calls, and only for long ones. Asserts both halves, because a static transcript is only a problem if PTY is what carries the anchor — if the transcript starts growing, the gate says to re-measure rather than trust the reasoning recorded in `pipeline_interactive.go`. |
