...
#   GUEST_NATS_URL endpoint injected as APE_NATS_URL   (default nats://aped.internal:4222)
#   IDLE_STOP      auto-stop idle workspaces after this (default empty = reaper off)
set -euo pipefail
...
# cannot read a home — and widening a home for a daemon is the wrong trade.
CRED_ROOT="${CRED_ROOT:-/srv/ape-credentials}"
# Credential mode composed into workspaces: oauth uses the published credential.
CREDENTIALS="${CREDENTIALS:-oauth}"
# The NATS endpoint workspaces are handed as APE_NATS_URL (PLAN-24 D5). Setting it
# is also what enables per-VM credential minting, so an empty value means guests
# boot with no agent — which is a supported configuration, not a broken one.
#
# It is a SENTINEL NAME and must stay one. The front resolves it to its own
# loopback listener through a system route in each workspace's CONNECT proxy; a
# loopback literal here would address the GUEST's loopback instead, and aped
# refuses to start rather than hand a guest an address that means something else
# where it lands.
GUEST_NATS_URL="${GUEST_NATS_URL:-nats://aped.internal:4222}"
# Idle-stop threshold for the node's reaper (PLAN-24 D7), e.g. 2h. EMPTY = off,
# which is the default: automatic lifecycle action is opt-in, so a redeploy must
# never start stopping workspaces on its own. The reaper also never touches a
# workspace it has no heartbeat for, so enabling it cannot reap one whose agent
# is absent.
IDLE_STOP="${IDLE_STOP:-}"
...
# install_exec_dropins writes the two ExecStart drop-ins, including ONLY the flags the
# installed binary actually understands.
#
# Built flag by flag on purpose. An all-or-nothing probe ("does it know --policy?")
# already failed twice here: the drop-in referenced a NEWER flag the gate did not
# check (--netd-socket, then --credentials) and the daemon died with "unknown flag" on
# start. Per-flag assembly makes that impossible — a flag the binary lacks simply is
# not written, the daemon keeps running with less capability, and `redeploy` fills it
# in once the matching binary is installed.
install_exec_dropins() {
  ...
  # ---- front ----
  local front_line="/usr/local/bin/aped front --node %H --state-dir /var/lib/aped --mgmt-host 127.0.0.1 --mgmt-port 4223 --operator-creds /var/lib/aped/creds/operator.creds"
  aped_supports front --policy \
    && front_line="$front_line --policy /etc/aped/policy.yaml" \
    || skipped="$skipped --policy"
  aped_supports front --framework-root \
    && front_line="$front_line --framework-root $FW_ROOT" \
    || skipped="$skipped --framework-root"
  # An EMPTY FW_REF must not emit a bare --framework-ref: systemd would take the next
  # flag as its value.
  ...
  aped_supports front --cache-root \
    ...
    || skipped="$skipped --cache-root"
  # --host-home predates all of this, but keep it probed for uniformity.
  aped_supports front --host-home \
    && front_line="$front_line --host-home ${CRED_ROOT}/${OP_USER:-root}" \
    || skipped="$skipped --host-home"
  aped_supports front --credentials \
    && front_line="$front_line --credentials $CREDENTIALS" \
    || skipped="$skipped --credentials"
  # The in-guest agent's endpoint (PLAN-24 D5). A non-empty value is also what
  # switches per-VM credential injection on — that half has been built and dormant
  # since PLAN-18 D6, waiting for an endpoint to exist.
  #
  # The value is a SENTINEL NAME, never a loopback literal: inside a guest,
  # 127.0.0.1 is the guest's own loopback. The front resolves this authority to its
  # own management listener through a system route in each workspace's CONNECT
  # proxy, so no new listener binds and no firewall hole opens.
  aped_supports front --guest-nats-url \
    && front_line="$front_line --guest-nats-url $GUEST_NATS_URL" \
    || skipped="$skipped --guest-nats-url"
  # The idle reaper (PLAN-24 D7). Empty by default — automatic lifecycle action is
  # opt-in, so a redeploy never starts stopping a node's workspaces on its own.
  if [ -n "$IDLE_STOP" ] && aped_supports front --idle-stop; then
    front_line="$front_line --idle-stop $IDLE_STOP"
  fi

  install_dropin aped-front.service 10-sandbox-roots.conf \
    "# Generated by deploy/dev-host.sh — policy + framework + caches + credentials." \
    "[Service]" \
    "# The credential root must be WRITABLE, not merely readable: ProtectSystem=strict makes" \
    "# everything outside ReadWritePaths read-only, and the shared-credential syncer writes" \
    "# the published credential in place when a workspace refreshes its token. Without this" \
    "# the sync fails with 'read-only file system' and workspaces silently drift apart." \
    "ReadWritePaths=$CRED_ROOT" \
    "ExecStart=" \
    "ExecStart=$front_line"

  [ -z "$skipped" ] || warn "installed aped predates:${skipped} — those features stay off until the redeploy verb runs"
}
...
EOF
  ...
  install -d -m 0750 "$CRED_ROOT"
  if [ -n "$OP_USER" ]; then
    chown "$OP_USER:ape" "$CRED_ROOT"
    install -d -m 0750 -o "$OP_USER" -g ape "$CRED_ROOT/$OP_USER"
    ok "$CRED_ROOT/$OP_USER (owner $OP_USER, group ape)"
  else
    ok "$CRED_ROOT"
  fi
