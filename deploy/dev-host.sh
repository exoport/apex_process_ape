...
install_exec_dropins() {
  ...
  if aped_supports front --policy && aped_supports front --framework-root; then
    ...
    install_dropin aped-front.service 10-sandbox-roots.conf \
      ...
      "[Service]" \
      "ExecStart=" \
      "ExecStart=/usr/local/bin/aped front --node %H --state-dir /var/lib/aped --mgmt-host 127.0.0.1 --mgmt-port 4223 --operator-creds /var/lib/aped/creds/operator.creds --policy /etc/aped/policy.yaml --framework-root ${FW_ROOT}${front_ref_flag} --cache-root $CACHE_ROOT --host-home ${CRED_ROOT}/${OP_USER:-root} --credentials $CREDENTIALS"
  else
