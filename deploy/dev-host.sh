...
BindReadOnlyPaths=$PROJECT_ROOT"
  ...
  # The executor drop-in: this box validated the containerd driver (--driver
  # containerd), and egress additionally needs the netns helper socket. Both live in
  # ONE generated drop-in so a redeploy cannot lose half of it.
  install_dropin aped.service 10-driver.conf \
    "# Generated by deploy/dev-host.sh — validated containerd driver + egress helper." \
    "[Service]" \
    "ExecStart=" \
    "ExecStart=/usr/local/bin/aped run --policy /etc/aped/policy.yaml --state-dir /var/lib/aped --audit-log /var/log/aped/audit.jsonl --node %H --allow-user aped --driver containerd --netd-socket /run/aped/netd.sock"

  # The front drop-in: point it at the policy (egress), the framework root, and the
  # durable tool caches. These are node-local paths, so they live in a generated
  # drop-in rather than the shipped unit.
  install_dropin aped-front.service 10-sandbox-roots.conf \
    "# Generated by deploy/dev-host.sh — policy + framework root + tool caches." \
    "[Service]" \
    "ExecStart=" \
    "ExecStart=/usr/local/bin/aped front --node %H --state-dir /var/lib/aped --mgmt-host 127.0.0.1 --mgmt-port 4223 --operator-creds /var/lib/aped/creds/operator.creds --policy /etc/aped/policy.yaml --framework-root $FW_ROOT --framework-ref ${FW_REF} --cache-root $CACHE_ROOT"
